-- Create comprehensive database schema for TouchPay POS (Fixed version)
...
-- Restaurant settings (using TEXT for updated_by to avoid UUID constraint)
...
-- Create RLS policies (permissive for demo purposes)
CREATE POLICY "Enable all access for all users" ON public.customer_profiles FOR ALL USING (true);
CREATE POLICY "Enable all access for all users" ON public.order_notes FOR ALL USING (true);
CREATE POLICY "Enable all access for all users" ON public.product_images FOR ALL USING (true);
CREATE POLICY "Enable all access for all users" ON public.promo_codes FOR ALL USING (true);
...
INSERT INTO public.restaurant_settings (setting_key, setting_value, data_type, description) VALUES
('tax_rate', '11', 'number', 'Tax rate percentage (PPN)'),
('service_charge_rate', '5', 'number', 'Service charge percentage'),
('tax_enabled', 'true', 'boolean', 'Enable tax calculation'),
('service_charge_enabled', 'false', 'boolean', 'Enable service charge'),
('auto_print_receipt', 'true', 'boolean', 'Automatically print receipts'),
('receipt_logo_url', '', 'string', 'URL for receipt logo');
...
CREATE POLICY "Users can upload menu images" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'menu-images');
CREATE POLICY "Users can update menu images" ON storage.objects FOR UPDATE USING (bucket_id = 'menu-images');
CREATE POLICY "Users can delete menu images" ON storage.objects FOR DELETE USING (bucket_id = 'menu-images');

CREATE POLICY "Users can view receipt PDFs" ON storage.objects FOR SELECT USING (bucket_id = 'receipt-pdfs');
CREATE POLICY "Users can upload receipt PDFs" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'receipt-pdfs');
